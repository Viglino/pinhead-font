...
var fontName = 'pinhead-font';    // set name of your symbol font
const className = 'pin'        // set class name in your CSS
...
/** Get glyphs */
var font = JSON.parse(fs.readFileSync('./pinhead-font.json'));
...
gulp.task('store', function(){
  return gulp.src(['svg/**/u*.svg'])
    .pipe(rename(function (path) {
      path.dirname = '';
      path.basename = 'pin-' + path.basename.replace(/u([^-]*)-/,'');
    }))
    .pipe(svgmin({
      plugins: [{
    ...
    .pipe(cheerio({
      run: function ($) {
        ...
        $('symbol').attr('viewBox', '0 0 15 15');
        $('symbol').attr('aria-hidden', 'true');
      },
      parserOptions: { xmlMode: true }
    }))
    .pipe(rename(function (path) {
      path.basename = 'pinhead-font';
    }))
    .pipe(gulp.dest('./dist'));
});
