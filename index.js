...
function setUrl(search, select) {
  ...
  if (typeof(select) === 'string') {
    if (select) opt.push('fg='+select.replace(/^pin-/,''));
  } else {
...
function showIcon(icon, url) {
  if (icon && icon.code) {
    currentGlyph = icon;
    $('#icon i').removeClass().addClass('pin-'+icon.name)
    $('#icon h2').text('pin-' + icon.name);
...
$.ajax({
  ...
  success: function(font) {
    ...
    if (newDate < new Date('2026-03-01')) {
      newDate = new Date('2026-03-01');
    }
    ...
    Object.keys(themes).forEach(function(th) {
      ...
      $('<h2>').text(th.replace('edit', 'edition').replace('geom', 'geometry').replace(/_/g, ' ')).appendTo(div);
      ...
      themes[th].forEach(function(gly) {
        ...
        $('<p>')
          ...
          .addClass(isNew ? 'new' : '')
          .append($('<span>').text(gly.name.replace(/_/g, ' ')))
          .attr('title', gly.name.replace(/_/g, ' ') + ' - ' + '\\'+gly.code.toString(16))
          .appendTo(div);
      });
    })
    ...
    search(data.q);
    if (data.fg) {
      showIcon(glyphs['pin-'+data.fg]);
      showPage('icons');
    } else {
      showIcon(glyphs['pin-pin'], false);
    }
  }
});